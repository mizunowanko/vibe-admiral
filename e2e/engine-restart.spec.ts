...
const SHIP_B = {
  id: "e2e-restart-ship-bbb",
  repo: "test-org/test-repo",
  issueNumber: 302,
  issueTitle: "Restart test ship B",
  branchName: "feature/302-restart-b",
};
...
test.describe("Engine Restart — WS Reconnection", () => {
  ...
  test("WS disconnect and reconnect restores connection indicator", async ({
    ...
    // Force WS close to simulate disconnect
    await page.evaluate(() => {
  ...
  test("Ship state displayed after page reload", async ({
    ...
    // Inject ships into the frontend
    ...
    await expect(
      page.getByText(`#${SHIP_A.issueNumber}`),
    ).toBeVisible({ timeout: 10_000 });

    // Note: After page reload, ships come from Engine's state sync,
    // not from WS injection. WS-injected ships won't persist across reload.
