...
# /audit-token — 総合トークン消費監査 & 節約 Issue 起票
...
curl -sf "http://localhost:$VIBE_ADMIRAL_ENGINE_PORT/api/dispatches?fleetId=<fleet-id>" | \
  python3 -c "import sys,json; data=json.load(sys.stdin); [print(f'{d[\"name\"]}: {d[\"status\"]}') for d in data.get('dispatches',[]) if d['name'].startswith('audit-token-')]"
```
...
```bash
curl -sf "http://localhost:$VIBE_ADMIRAL_ENGINE_PORT/api/dispatches?fleetId=<fleet-id>" | \
  python3 -c "import sys,json; data=json.load(sys.stdin); [print(f'=== {d[\"name\"]} ===\n{d.get(\"result\",\"(no result)\")}') for d in data.get('dispatches',[]) if d['name'].startswith('audit-token-') and d['status']=='completed']"
```
...
- #912 — /audit-token 総合監査スキルによる調査結果
ISSUEEOF
)"
```
