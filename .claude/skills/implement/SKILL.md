...
GitHub Issues をベースに、計画→実装→受け入れテスト→マージまでを一気通貫で行うスキル。
...
| Step | ステータス (`status`) |
|------|---------------------|
| 3 (調査) | (planning 継続、遷移表明なし) |
| 4 (計画) | `planning` (初期ステータス) |
| 5 (実装) | `implementing` |
| 6 (ビルド検証) | (implementing 継続、遷移表明なし) |
| 8 (テスト再実行) | (implementing 継続、遷移表明なし) |
| 9 (コミット & PR) | `acceptance-test` (PR 作成後に遷移) |
| 11 (受け入れテスト) | (acceptance-test 継続、遷移表明なし) |
| 15 (マージ) | `merging` |
| 15.5 (完了) | `done` |
...
PR 作成後、`acceptance-test` への遷移を表明する。この遷移は `implementing → acceptance-test` の code-review Gate をトリガーする:
...
Step 9 で `implementing → acceptance-test` の遷移を表明した際に code-review Gate が発動し、Bridge が自動的に PR のコードレビューを実施する。
Ship は Gate 待機フロー（前述）に従い、`gate-response.json` を待機する。

- `approved: true` → Step 11 へ進む
- `approved: false` → Bridge のフィードバックは PR レビューコメントに記録されている。`gh pr view <pr-number> --repo "$REPO" --json reviews,comments` で確認し、修正 → commit & push → 再度 `status-transition` で `acceptance-test` を表明
...
dev サーバーの起動、acceptance-test-request.json の作成、ファイル伝言板の待機はすべて不要。

#### VIBE_ADMIRAL 設定時（ファイル伝言板方式）

Step 9 で `acceptance-test` への遷移は既に完了している。受け入れテストのファイル伝言板フローに進む。
...
まず `merging` への遷移を表明する。この遷移は `acceptance-test → merging` の playwright Gate をトリガーする（**ただし Step 4 で `qaRequired: false` と判断された場合、Engine が playwright Gate を自動スキップし `ok: true` が即座に返る**）:
