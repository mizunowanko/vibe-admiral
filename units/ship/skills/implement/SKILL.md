...
while [ $ELAPSED -lt $TIMEOUT ]; do
  RESULT=$(curl -sS --fail-with-body --max-time 130 "http://localhost:${VIBE_ADMIRAL_ENGINE_PORT:-9721}/api/ship/${VIBE_ADMIRAL_SHIP_ID}/phase/wait?currentPhase=<current-gate-phase>")
...
```bash
TRANSITION_JSON=$(curl -sS --fail-with-body "http://localhost:${VIBE_ADMIRAL_ENGINE_PORT:-9721}/api/ship/${VIBE_ADMIRAL_SHIP_ID}/phase-transition-log?limit=1")
...
| `qa → qa-gate` | Fleet 設定による | Ship の Escort が受け入れテストを実行（Fleet `gatePrompts` で手順をカスタマイズ。未設定時は auto-approve。`qaRequired: false` の場合スキップ。Fleet `acceptanceTestRequired: false` の場合も Escort が即 auto-approve） |
