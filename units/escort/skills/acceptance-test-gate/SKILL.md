...
### Step 4: GitHub コメント → Gate intent → verdict

Gate API は親 Ship（`PARENT_SHIP_ID`）に対して実行する。`SHIP_ID`（Escort 自身）ではない。

**4a. 結果を PR コメントとして書き込む（verdict より先に実行）**:

```bash
COMMENT_URL=$(gh pr comment <PR_NUMBER> --repo "$REPO" --body "## Acceptance Test Result: ✅ PASS / ❌ FAIL
