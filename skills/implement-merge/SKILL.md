...
2. DB で応答メッセージをポーリング:
   ...
   while true; do
     ROW=$(sqlite3 "$DB_PATH" "SELECT payload FROM messages WHERE ship_id='$SHIP_ID' AND type='acceptance-test-response' AND read_at IS NULL LIMIT 1" 2>/dev/null)
     if [ -n "$ROW" ]; then
       sqlite3 "$DB_PATH" "UPDATE messages SET read_at=datetime('now') WHERE ship_id='$SHIP_ID' AND type='acceptance-test-response' AND read_at IS NULL"
       echo "$ROW"
       break
     fi
     sleep 2
