...
調査の結果、以下のいずれかに該当する場合は「何もすることがない」と判断する:

- Issue で指摘された問題が既にデフォルトブランチで修正済み（コードに差分がない）
- 該当するバグが現在のコードでは再現しない
- 別の PR/Ship が既に同じ変更をマージ済み

**`VIBE_ADMIRAL` 設定時**: admiral-request で Engine に通知し、ワークフローを終了する:

````
```admiral-request
{ "request": "nothing-to-do", "reason": "<理由の要約（英語）>" }
```
````
...
Engine が `ok: true` を返したら、workflow-state.json を削除してワークフローを終了する:

```bash
rm -f .claude/workflow-state.json
```

**`VIBE_ADMIRAL` 未設定時**: Issue にコメントを残して close し、worktree を削除してワークフローを終了する:

```bash
gh issue comment <ISSUE_NUMBER> --repo "$REPO" --body "Investigation complete: nothing to do. <理由>"
gh issue close <ISSUE_NUMBER> --repo "$REPO"
```

何かしらの実装が必要な場合は Step 4 に進む。
