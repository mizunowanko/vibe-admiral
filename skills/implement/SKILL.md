...
| `acceptance-test → merging` | auto-approve | 自動承認（デフォルト） |
...
- CI が失敗した場合は Step 15（CI 修正ループ）へ
...
Step 10 で code-review Gate の approve/reject 対応が完了しているため、そのまま Step 14 へ進む。
...
まず `merging` への遷移を表明する。この遷移は `acceptance-test → merging` の auto-approve Gate をトリガーする:
...
Gate が発動した場合は Gate 待機フローに従う。デフォルトは `auto-approve` のため即座に承認される。Gate 承認後、マージを実行する。
