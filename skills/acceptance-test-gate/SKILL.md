...
#### Fleet レベル auto-approve（`ACCEPTANCE_TEST_REQUIRED=false`）

`ACCEPTANCE_TEST_REQUIRED` が `false` の場合、Fleet 設定で受け入れテストが無効化されている。即 auto-approve:
...
1. PR を特定してコメントを投稿:
   ...
   ```bash
   gh pr comment "$PR_NUMBER" --repo "$REPO" --body "## Acceptance Test: ⏭️ SKIPPED (Fleet Setting)

   Fleet setting \`acceptanceTestRequired: false\` — acceptance testing disabled at fleet level.
   Code review gate still applies independently.
...
`ACCEPTANCE_TEST_REQUIRED` が `true`（デフォルト）の場合、以降の QA チェックに進む。
