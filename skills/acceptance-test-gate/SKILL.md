...
1. **フォールバックチェック**: `QA_REQUIRED_PATHS` が設定されている場合、実際の変更ファイルを確認する:
   ```bash
...
### Step 5: Gate intent を宣言（verdict 前のフォールバック用）
...
### Step 6: Gate verdict を送信（PR コメントより先に実行 — プロセス終了による verdict 喪失を防止）
