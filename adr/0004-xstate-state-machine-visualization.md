...
| **失敗時** | Phase mismatch → 400 エラー、遷移なし / 重複遷移 → no-op（冪等） |
...
| **副作用（Actions）** | - XState: `phaseBeforeStopped` に現在の phase を保存<br>- Engine: Ship プロセスを kill<br>- DB: `ships.phase` → `stopped`、`phase_transitions` に audit log 挿入<br>- Frontend: `ship:status` broadcast で UI 更新 |
...
| **副作用（Actions）** | - XState: `processDead = false`、`retryCount++`<br>- Engine: 旧プロセスを kill（まだ残っている場合）、新しい Ship プロセスを起動<br>- DB: `ships.phase` → 復帰先 phase、`phase_transitions` に audit log 挿入<br>- Frontend: `ship:status` broadcast で UI 更新 |
...
| **副作用** | - XState: `clearGateCheck` + 終了コードをコンテキストに保存、前の作業 phase に遷移<br>- DB: `phase_transitions` に audit log 挿入（metadata に exit code）<br>- Frontend: `ship:status` broadcast でエラー情報を表示 |
